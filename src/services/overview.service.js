...
class OverviewService extends BaseService {
  ...
  async getOverview(userId) {
    ...
    // Fetch group details with a single call
    ...
    const recentGroupDetailsWithSettlement = await Promise.all(
      recentGroupDetails.map(async (grp) => ({
        ...grp.toObject(),
        settlement: await this.calculateGroupSettlement(grp._id)
      }))
    );
   ...
   */
  async calculateGroupSettlement(groupId) {
    ...
    return this.simplifyDebts(debtMap);
  }
